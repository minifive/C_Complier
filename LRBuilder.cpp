		...
		for (const auto& lrproduction : state.productions) {
			...
			if (lrproduction.pos < (int)prooduction.right.size()) { //dot is not at end
				...
				if (!tokenManager.isTerminal(nextToken)) {//.后面是非终结符，开始展开
					//youhua
					vector<Production> nextProdutions;
					if (productionbuffer.find((NonterminalToken&)nextToken) == productionbuffer.end()){
						nextProdutions = productionManager.getProductions((NonterminalToken&)nextToken);
						productionbuffer[(NonterminalToken&)nextToken] = nextProdutions;
					}
					else
						nextProdutions = productionbuffer[(NonterminalToken&)nextToken];
					...
					for (const auto& nextProdution : nextProdutions) {
						...
						vector<int> nextFirsts;
						if (firstbuffer.find(tokenids) == firstbuffer.end()){
							nextFirsts = getFirst(tokenids);
							firstbuffer[tokenids] = nextFirsts;
						}
						else
							nextFirsts = firstbuffer[tokenids];
